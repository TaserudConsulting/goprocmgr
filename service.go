...
type Services struct {
	config         *Config
	ActiveServices map[string]*ActiveService
}
...
func (services *Services) Start(name string) error {
	// Init the active services map
	if services.ActiveServices == nil {
		services.ActiveServices = make(map[string]*ActiveService)
	}

	// If the config for the service doesn't exist, return an error
	if _, ok := services.config.Servers[name]; !ok {
		return fmt.Errorf("unknown service: %s", name)
	}

	// If the service already exists, return an error
	if _, ok := services.ActiveServices[name]; ok {
		return fmt.Errorf("service is already running: %s", name)
	}

	// Randomize a port to supply as environment variable.
	port, err := services.randomizePortNumber()
	if err != nil {
		return err
	}

	// Set up the environment and set the port
	env := services.config.Servers[name].Environment
	env["PORT"] = fmt.Sprintf("%d", port)

	// Split out the arguments
	splitCmd := strings.Split(services.config.Servers[name].Command, " ")

	// Create the service config
	svcConfig := &service.Config{
		Name:        fmt.Sprintf("goprocmgr-%s", name),
		DisplayName: fmt.Sprintf("goprocmgr %s", name),
		Description: fmt.Sprintf("goprocmgr %s", name),
		Option: service.KeyValue{
			"UserService": true,
		},
		EnvVars:          env,
		WorkingDirectory: services.config.Servers[name].Directory,
		Executable:       splitCmd[0],
		Arguments:        splitCmd[1:],
	}
	...
	// Store the service for easy later reference use
	as.RunningService = &s

	// Install the service to the OS service manager
	err = s.Install()
	if err != nil {
		return err
	}

	// Start the service using the OS service manager
	err = s.Start()
	if err != nil {
		return err
	}
...
func (runner *Services) randomizePortNumber() (uint, error) {
	portRangeSize := int(runner.config.Settings.PortRangeMax - runner.config.Settings.PortRangeMin)

	if len(runner.ActiveServices) >= portRangeSize {
	...
	for _, randomPort := range randomPorts {
		...
		for _, activeProcess := range runner.ActiveServices {
...
func (as *ActiveService) Start(s service.Service) error {
...
func (as *ActiveService) run() error {
	fmt.Printf("I'm running %v\n", service.Platform())
	ticker := time.NewTicker(2 * time.Second)
	for {
		select {
		case tm := <-ticker.C:
			fmt.Printf("Still running at %s...\n", tm)
		case <-as.exit:
			ticker.Stop()
			return nil
		}
...
func (as *ActiveService) Stop(s service.Service) error {
	// Any work in Stop should be quick, usually a few seconds at most.
	fmt.Println("I'm Stopping!")
	close(as.exit)
	return nil
}
...
// 	}
//
// 	fmt.Println("svcConfig", svcConfig)
//
// 	// Randomize a port to supply as environment variable.
...
// 	}
//
// 	// Set up active runner
// 	activeRunner := &ActiveRunner{
// 		Port:         port,
// 		ServerConfig: runner.config.Servers[name],
// 	}
//
// 	// Create the service
// 	s, err := service.New(activeRunner, svcConfig)
// 	if err != nil {
// 		return err
// 	}
//
...
// }
//
// func (ar *ActiveRunner) Start(s service.Service) error {
// 	go ar.run()
//
// 	return nil
// }
//
// func (ar *ActiveRunner) run() {
// 	fmt.Println("Running")
// }
//
// func (ar *ActiveRunner) Stop(s service.Service) error {
// 	return nil
// }
//
